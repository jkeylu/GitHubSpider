...
Db.prototype.createIndex = function() {
  ...
  var createIndexCb = function(err, indexName) {
    if (err) {
      console.error('Db - createIndex() %s', err);
      return;
    }
    ...
    async.parallel([
      function(cb) {
        var filter = { id: repo.id }
          , update = { $set: repo, $unset: { deleted_at: '' } };
        repositories.updateOne(filter, update, options, cb);
      },
      function (cb) {
        var filter = { id: owner.id }
          , update = { $set: owner };
        users.updateOne(filter, update, options, cb);
      }
    ], done);
  };
...
Db.prototype.updateRepoDetail = function(repo, callback) {
  var keys = [
    ...
    'open_issues_count', 'default_branch', 'network_count', 'subscribers_count'
  ];
  var item = pick(repo, keys);
  item.created_at = new Date(item.created_at);
  item.updated_at = new Date(item.updated_at);
  item.pushed_at = new Date(item.pushed_at);
  item.owner = pick(repo.owner, ['login', 'id', 'avatar_url', 'type']);
