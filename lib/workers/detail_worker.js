...
function updateRepoDetail(worker, fullname, callback) {
  async.parallel([
    function(cb) {
      var token = worker.app.tokenManager.get();
      if (!token) {
        return cb(new RateLimitExceededError());
      }

      var url = util.format('https://api.github.com/repos/%s', fullname);
      var options = { username: token };
      needle.get(url, options, function(err, resp, repo) {
      });
    },
    function(cb) {
      var token = worker.app.tokenManager.get();
