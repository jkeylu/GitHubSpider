...
DetailWorker.prototype.start = function() {
    ...
    updateNoDetailRepo(repo.full_name, self, function(err) {
      if (err) {
        if (err instanceof RateLimitExceededError) {
          ...
          console.log('DetailWorker - Repo id: %d, fullname: %s, %s.', repo.id, repo.full_name, err.message);
          ...
          console.error('DetailWorker - Unknown error: %s', err);
  ...
  var end = function(err) {
    if (err) {
      console.error('DetailWorker - end Error: %s', err);
      if (err instanceof RateLimitExceededError) {
        self._running = false;
        return;
      }
